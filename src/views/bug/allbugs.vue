  ...
  <div class="app-container">
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" border fit highlight-current-row
      ...
      <el-table-column min-width="150px"  align="center" :label="$t('table.title')">
        <template slot-scope="scope">
          <router-link class="link-type" :to="'/showbug/'+scope.row.id">
      ...
      <!--<el-table-column width="110px" v-if='showReviewer' align="center" :label="$t('table.reviewer')">-->
        ...
        <!--</template>-->
      <!--</el-table-column>-->
      <el-table-column width="80px" class-name="status-col" :label="$t('table.level')">
        <template slot-scope="scope">
          <span>{{scope.row.level}}</span>
          <!--<svg-icon v-for="n in +scope.row.importance" icon-class="star" class="meta-item__icon" :key="n"></svg-icon>-->
        </template>
      </el-table-column>
      <el-table-column width="80px" class-name="status-col" :label="$t('table.importance')">
        <template slot-scope="scope">
      ...
      <el-table-column class-name="status-col" :label="$t('table.env')" width="100">
        <template slot-scope="scope">
          <span>{{scope.row.env}}</span>
          <!--<el-tag :type="scope.row.status | statusFilter">{{scope.row.env}}</el-tag>-->
        </template>
      </el-table-column>
      <el-table-column align="center" :label="$t('table.handle')" width="95">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" :label="$t('table.actions')" width="230" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <!--<el-select-->
            ...
            <!--</el-option>-->
          <!--</el-select>-->
          <el-select  style="width: 200px" class="filter-item" v-model="scope.row.status" @change="changestatus(scope.row)" placeholder="修改状态">
            <el-option v-for="item,index in statuslist" :key="index" :label="item.value" :value="item.label">
            </el-option>
          </el-select>
