  ...
  <div class="app-container">
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" border fit highlight-current-row
      ...
      <!--<el-table-column align="center" :label="$t('table.actions')" width="230" class-name="small-padding fixed-width">-->
        <!--<template slot-scope="scope">-->
          <!--&lt;!&ndash;<el-button type="primary" size="mini" @click="handleUpdate(scope.row)">{{$t('table.edit')}}</el-button>&ndash;&gt;-->
          <!--&lt;!&ndash;<el-button  size="mini" type="success" @click="handleModifyStatus(scope.row)">{{$t('table.publish')}}&ndash;&gt;-->
          ...
          <!--&lt;!&ndash;</el-button>&ndash;&gt;-->
          <!--&lt;!&ndash;<el-button v-if="scope.row.status!='deleted'" size="mini" type="danger" @click="handleModifyStatus(scope.row,'deleted')">{{$t('table.delete')}}&ndash;&gt;-->
          <!--&lt;!&ndash;</el-button>&ndash;&gt;-->
        <!--</template>-->
      <!--</el-table-column>-->
    </el-table>
...
import { getcompletelist, gettasklist, changetask } from '@/api/table'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getcompletelist(getProject()).then(response => {
    ...
    handleModifyStatus(row) {
      const sl = this.status.length
      console.log(this.status)
      console.log(row.status)
      for (let i = 0; i < sl; i++) {
        if (this.status[i].taskstatus === row.status) {
          row.status = this.status[i + 1].taskstatus
          changetask(row.id).then(resp => {
            if (resp.data === 'ok') {
              this.$message({
                message: '操作成功',
                type: 'success'
